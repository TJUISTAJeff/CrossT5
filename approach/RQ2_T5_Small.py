...
def train():
    ...
    model = ExtraT5ForConditionalGeneration(model_config, t5_model_path = "../outmodels/T5_Small_Best.pt", model_file_path = "../outmodels/hidden_size_128_back_up.pt")
    ...
    ori_train = readpickle(os.path.join(config.data_dir, 'processtraindata_all.pkl'))
    ...
    for epoch in range(config.train_iters):
        ...
        for batch_inst in tqdm(dataloader):
            ...
            if use_cuda:
                for x in batch_inst:
                    if x in ['para_batch_1', 'para_batch_2', 'seq_len_batch_1', 'seq_len_batch_2']:
                        batch_inst[x][0] = batch_inst[x][0].to(device)
                        batch_inst[x][1] = batch_inst[x][1].to(device)
                        batch_inst[x][2] = batch_inst[x][2].to(device)
                        batch_inst[x][3] = batch_inst[x][3].to(device)
                        batch_inst[x][4] = batch_inst[x][4].to(device)
                    else:
                        batch_inst[x] = batch_inst[x].to(device)
            output = model(input_ids=batch_inst,
