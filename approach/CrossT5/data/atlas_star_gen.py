...
def get_type(assertion_type, assertion, arg, full_test):
    full_test = "public " + full_test.replace("\"<AssertPlaceHolder>\" ; }", " ")
    ...
    if assertion_type == "assertNotNull" or assertion_type == "assertNull" and isinstance(arg,
...
def get_type_info(assertion, test_method):
    ...
    try:
        ...
        if assertion.startswith("this"):
            errs["begin_with_this"] += 1
        return False
...
def gen_varients(_type, arg, matching_type_vars):
    ...
    if _type in vocab:
        ...
        if _type == int:
            ...
            top_values = [float(x.replace("(", "").replace(")", "").replace(" ", "").replace('Complex.', '')) for x in
                          top_values]
        elif _type == str:
...
def get_data(assertion_file, method_file):
    ...
    assertions = open(assertion_file, 'r', encoding='utf-8').read().split("\n")

    print(f'processing {len(assertions)} samples')

    n_type_inferred = 0

    methods = open(method_file, 'r', encoding='utf-8').read().split("\n")
    ...
    for idx, (assertion, method) in tqdm.tqdm(enumerate(zip(assertions, methods))):
        ...
        if assertion[:-8] not in template_asserts:
            errs["assertion-not-in-templates"] += 1
            valid_idx.append([idx, 0])
            continue

        assertion_clean = clean(assertion[:-8])
    ...
    return method_test_assert_data, included_idxs, aligned_included_idxs, atlas_star, valid_idx


if __name__ == "__main__":
    random.seed(0)
    # os.environ["PROJECT_PATH"] = "F:\Code\Python\DeepOracle"
    #
    # Project_path = os.environ["PROJECT_PATH"]
    # DATA_DIR = os.path.join(Project_path, 'datasets', "Our_Toga_Transformed_Dataset")
    DATA_DIR = sys.argv[1]
    vocab_src = np.load(os.path.join("/home/icse2022_artifact/data", "vocab.npy"), allow_pickle=True).item()

    # DATA_DIR = "F:\\Code\\Python\DeepOracle\\datasets\\study_datasets\\file_Dataset"
    # out_dir = "F:\\Code\\Python\DeepOracle\\datasets\\study_datasets\\file_star"
    # vocab_src = np.load("F:\\Code\\Python\\DeepOracle\\datasets\\Our_Toga_Transformed_Dataset\\vocab.npy",
    ...
    # for split, split_n in zip(["our_toga_test"], ['our_toga_test']):
        ...
        local_method_test_assert_data, included_idxs, aligned_included_idxs, atlas_star, valid_idx = get_data(
            f_assertion,
            f_method)

        data_dir = os.path.join(DATA_DIR, f'our_data/')
        try:
            os.makedirs(data_dir)
        except:
            pass

        with open(os.path.join(DATA_DIR, f'our_data/{split_n}.csv'), "w", encoding='utf-8') as f:
            w = csv.writer(f)
            w.writerow(["idx", "label", "fm", "test", "assertion"])
            for idx, d in enumerate(local_method_test_assert_data):
                assert len(list(d)) == 4
                w.writerow([idx] + list(d))
        with open(os.path.join(DATA_DIR, f'our_data/{split_n}_valid_idx.txt'), 'w', encoding="utf-8") as f:
            for i in valid_idx:
                f.write(str(i) + '\n')

        print(f'wrote to our_data/{split_n}.csv')
        print(errs)
        ...
        with open(os.path.join(DATA_DIR, f'our_data/{split_n}.oov_asserts.txt'), 'w') as f:
